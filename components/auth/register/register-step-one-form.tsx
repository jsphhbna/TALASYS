...
import { parseIdImage } from "@/lib/resident"
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {formData.password.length === 16 && (
          <p className="text-amber-500 text-xs mt-1 font-medium">⚠️ 16 character limit reached</p>
        )}
        <div className="mt-2 text-xs">
          <p className="font-medium text-slate-700 dark:text-slate-300 mb-1.5">Password must contain:</p>
          <div className="flex flex-col gap-1 text-slate-600 dark:text-slate-400">
            {getPasswordRequirements().map((req, idx) => (
              <div key={idx} className={`flex items-center gap-1.5 ${req.met ? 'text-green-600 font-medium' : ''}`}>
                {req.met ? (
      ...
      <p className="text-center text-sm text-slate-600 dark:text-slate-400">
        ...
        <a href="/auth/login" className="text-[#0C2340] dark:text-blue-50 font-semibold hover:underline">
          Sign In
        </a>
      </p>
    </div>
