...
export function ProfileAccountInfoCard({ user }: ProfileAccountInfoCardProps) {
  ...
  return (
    <Card className="p-6">
      <h3 className="text-base font-semibold text-[#0C2340] dark:text-blue-50 mb-5">Account Information</h3>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div>
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-1">Account Created</p>
          <p className="text-sm text-slate-900 dark:text-slate-100">{formatAccountCreated(user.createdAt)}</p>
        </div>

        <div>
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-1">Account Expiry</p>
          <p className="text-sm text-slate-900 dark:text-slate-100">{user.accountExpiry || "N/A"}</p>
        </div>
      ...
      {(isExpiring || isExpired) && (
        <div className="mt-8 border-t border-slate-100 dark:border-slate-800 pt-6">
          <h4 className="text-sm font-semibold text-[#0C2340] dark:text-blue-50 mb-4">Reactivation Process</h4>
              ...
              <div className="w-full relative group">
                <button
                  ...
                  className={`w-full py-2.5 px-4 text-sm font-medium rounded transition-colors ${
                    isExpired
                      ? "bg-[#0C2340] dark:bg-slate-800 text-white hover:bg-[#1a3a5c]"
